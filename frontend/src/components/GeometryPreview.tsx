...
export const GeometryPreview: React.FC<GeometryPreviewProps> = ({ project: model, className, boundingBox, volumeCm3, featured=false, summary }) => {
  ...
  const body=<>{controls}{canvas}{info}</>;
  if(featured) return <div className={className+' geometry-preview-featured'} data-testid="shared-geometry-preview"><div className="geometry-preview-featured-sidebar">{controls}{info}{summary}</div><div className="geometry-preview-featured-canvas">{canvas}</div></div>;
