...
class TestS104ExactComplexProfileTracing:
    ...
    def test_scale_confirmation_gate(self):
        """Approve interface fails if scale is unconfirmed, succeeds once confirmed."""
        app = create_app()
        ...
        img_bytes = create_test_extrusion_png()
        client.post(
            f"/api/projects/{pid}/interfaces/interface_a/image",
            files={"file": ("drawing_a_2040_t_slot.png", img_bytes, "image/png")},
        ...
        # Set profile_type to traced_closed and scale unconfirmed
        client.patch(
        ...
        assert "Scale calibration must be confirmed" in appr_resp.json()["error"]["message"]
