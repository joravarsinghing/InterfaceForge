...
def test_lite_success_without_fallback() -> None:
    """Test Lite primary model returns valid result without triggering fallback."""
    provider = GeminiAnalysisProvider(api_key="test_api_key")
    valid_json = """
    {
      "profile_type": "circle",
      "candidate_points": [{"x": -25.0, "y": 0.0}, {"x": 25.0, "y": 0.0}],
...
def test_malformed_lite_response_triggers_flash() -> None:
    """Test malformed response from Lite model triggers Flash fallback."""
    provider = GeminiAnalysisProvider(api_key="test_api_key")
    malformed_json = "{ invalid json payload"
    valid_flash_json = """
    {
      "profile_type": "rectangle",
      "candidate_points": [{"x": -30.0, "y": -20.0}],
      "candidate_dimensions": [],
      "provenance": "image_extracted",
      "confidence": 0.90,
      "warnings": [],
    ...
    resp_flash.usage_metadata.total_token_count = 250
    ...
    with patch("google.genai.Client") as mock_client_cls:
        ...
        result = provider.analyze(b"fake_image_bytes", "rect.png")

        assert result.profile_type == ProfileType.RECTANGLE
        assert result.model_used == "gemini-3.6-flash"
        assert result.fallback_triggered is True
        assert mock_client.models.generate_content.call_count == 2
        first_model_call = mock_client.models.generate_content.call_args_list[0].kwargs["model"]
        second_model_call = mock_client.models.generate_content.call_args_list[1].kwargs["model"]
        assert first_model_call == "gemini-3.5-flash-lite"
        assert second_model_call == "gemini-3.6-flash"
